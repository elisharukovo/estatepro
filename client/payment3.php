...
<div class="super_container">
	...
	<form style="display:none" role="form" id="bank" class="form-horizontal" action="<?php $_PHP_SELF ?>" method="POST" target="my-iframe">
		...
		<div class="row form-group">
			   ...
			   <input type="number" id="amount" name="amount" class="form-control" min="1" max="<?php echo get_balance($id) ?>" required>
			   </div>
        </div>
	...
	<form role="form" id="mobile" class="form-horizontal" action="<?php $_PHP_SELF ?>" method="POST" target="my-iframe">
...
<?php
    ...
    <script type="text/javascript">
	    ...
	    if($amount<$balance)
	    {
			$sql1 = "insert into payments(username,house_id,payment_method,amount_paid) values('$_SESSION[client]','$id','$plan','$amount')";
			$result1 = mysqli_query($dbcon, $sql1) or die(mysqli_error($dbcon));
			
			$balance=get_balance($id);
			
			$sql2 = "Insert into ozekimessageout (receiver,msg,status) values ('$contact', 'Your payment of $ $amount have been successfully received. Your balance is now $ $balance.','send')";
			$result2 = mysqli_query($dbcon, $sql2) or die(mysqli_error($dbcon));
	 
            ?>
            <script language="javascript">
	            alert("Your payment of $ <?php echo $amount; ?> have been received");
                parent.location = 'houses_purchased.php';
            </script>
            <?php
	    }
		else
		{
			...
			$sql2 = "update houses set status=1 where id='$id'";
			$result2 = mysqli_query($dbcon, $sql2) or die(mysqli_error($dbcon));
			
			$sql3 = "Insert into ozekimessageout (receiver,msg,status) values ('$contact', 'Your payment of $ $amount have been successfully received. You have now completed your payments. Your Title deeds are being processed','send')";
    ...
    if(isset($_POST['submitBank']))
	{
		...
		if($amount<$balance)
	    {
			$sql1 = "insert into payments(username,house_id,payment_method,amount_paid) values('$_SESSION[client]','$id','Bank Transfer','$amount')";
