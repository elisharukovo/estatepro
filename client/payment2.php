...
<div class="super_container">
	...
	<form style="display:none" role="form" id="bank" class="form-horizontal" action="<?php $_PHP_SELF ?>" method="POST">
		...
		<div class="row form-group">
			   ...
			   <input type="number" id="amount" name="amount" class="form-control" min="<?php echo $deposit ?>" value="<?php echo $deposit ?>" required>
			   </div>
        </div>
...
<?php
	...
	$sql2 .= "update houses set status=2, username='$_SESSION[client]', date_purchased=now() where id='$id'";
	...
	$sql3 = "Insert into ozekimessageout (receiver,msg,status) 
		         values 
			('$contact', 'Congratulations! for buying a house. Your deposit of $ $amount have been successfully received. Your balance is now $ $balance. Your title deeds will be processed after last instalment','send')";
	$result3 = mysqli_query($dbcon, $sql3) or die(mysqli_error($dbcon));
	?>
    <script language="javascript">
	    alert("Congratulations! for buying a house. Your deposit of $ <?php echo $amount; ?> have been successfully received");
