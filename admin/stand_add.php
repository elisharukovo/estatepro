	...
	<div class="alert alert-info"><Strong>Add New Stand</strong></div>
	
	<form action="<?php $_PHP_SELF ?>" method="POST" class="form-horizontal" enctype="multipart/form-data">
        
		<div class="row form-group">
               <div class="col col-md-3"><label for="town" class="form-control-label">Town</label></div>
               <div class="col-12 col-md-9"><input type="text" id="town" name="town" class="form-control" required></div>
        </div>
...
<?php
     ...
     $query2 = "insert into stands(image, surburb, description, price, stand_size)
					values('$newname','$surburb','$description','$price','$stand_size')";
     $result2 = mysqli_query($dbcon, $query2) or die(mysqli_error($dbcon));
	 
	 $last_id = mysqli_insert_id($dbcon);
	 
	 $query3 = "insert into images(stand_id,image_name) values('$last_id','$newname')";
     ...
     alert("Stand successfully Uploaded");
